...
st.write('The ticker symbols for Euronext include a suffix that indicates the exchange where the stock is primarily listed.')
st.markdown('- .PA indicates Paris')
st.markdown('- .AS indicates Amsterdam')
st.markdown('- .BR indicates Brussels')

# Define the sectors and their corresponding main tickets
sectors = {
    'Finance': ['BNP.PA', 'INGA.AS', 'ASML.AS', 'AD.AS'],
    'Industrials': ['AIR.PA', 'SAF.PA', 'AF.AS', 'AKZO.AS'],
    'Consumer Discretionary': ['LVMH.PA', 'KER.PA', 'CP.PA', 'RM.PA'],
    'Health Care': ['SAN.PA', 'NOVO.CO', 'ASM.AS', 'GSK.L'],
    'Consumer Staples': ['UN.AS', 'HEI.AS', 'AN.AS', 'DLB.AS'],
    'Energy': ['TTE.AS', 'RDSB.AS', 'OG.AS', 'SHEL.L'],
    'Basic Materials': ['AIRP.PA', 'ERA.PA', 'ARCE.AS', 'NXPE.PA'],
    'Technology': ['STM.PA', 'CAP.PA', 'SOI.PA', 'ATOS.PA'],
    'Utilities': ['ENGI.PA', 'EDF.PA', 'RWE.DE', 'EON.DE'],
    'Real Estate': ['UNI.AS', 'KLPR.AS', 'VSTE.AS', 'WER.AS'],
}
